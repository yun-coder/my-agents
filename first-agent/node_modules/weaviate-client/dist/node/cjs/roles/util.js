...
class PermissionGuards {
    static includes = (permission, ...actions) => actions.filter((a) => Array.from(permission.actions).includes(a)).length > 0;
    static isAlias = (permission) => PermissionGuards.includes(permission, 'create_aliases', 'read_aliases', 'update_aliases', 'delete_aliases');
    static isBackups = (permission) => PermissionGuards.includes(permission, 'manage_backups');
    static isCluster = (permission) => PermissionGuards.includes(permission, 'read_cluster');
    static isCollections = (permission) => PermissionGuards.includes(permission, 'create_collections', 'delete_collections', 'read_collections', 'update_collections');
    static isData = (permission) => PermissionGuards.includes(permission, 'create_data', 'delete_data', 'read_data', 'update_data');
    static isGroups = (permission) => PermissionGuards.includes(permission, 'read_groups', 'assign_and_revoke_groups');
    static isNodes = (permission) => PermissionGuards.includes(permission, 'read_nodes');
    static isReplicate = (permission) => PermissionGuards.includes(permission, 'create_replicate', 'read_replicate', 'update_replicate', 'delete_replicate');
    static isRoles = (permission) => PermissionGuards.includes(permission, 'create_roles', 'read_roles', 'update_roles', 'delete_roles');
    static isTenants = (permission) => PermissionGuards.includes(permission, 'create_tenants', 'delete_tenants', 'read_tenants', 'update_tenants');
    static isUsers = (permission) => PermissionGuards.includes(permission, 'read_users', 'assign_and_revoke_users');
    static isPermission = (permissions) => !Array.isArray(permissions);
    static isPermissionArray = (permissions) => Array.isArray(permissions) && permissions.every(PermissionGuards.isPermission);
    static isPermissionMatrix = (permissions) => Array.isArray(permissions) && permissions.every(PermissionGuards.isPermissionArray);
    static isPermissionTuple = (permissions) => Array.isArray(permissions) &&
...
class Map {
    static flattenPermissions = (permissions) => !Array.isArray(permissions) ? [permissions] : permissions.flat(2);
    static permissionToWeaviate = (permission) => {
    ...
    static roleFromWeaviate = (role) => PermissionsMapping.use(role).map();
    static roles = (roles) => roles.reduce((acc, role) => ({
        ...acc,
        [role.name]: Map.roleFromWeaviate(role),
    }), {});
    static groupsAssignments = (groups) => groups.map((g) => ({
        groupID: g.groupId || '',
        groupType: g.groupType,
    }));
    static users = (users) => users.reduce((acc, user) => ({
        ...acc,
        [user]: { id: user },
    }), {});
    static user = (user) => ({
    ...
    static dbUsers = (users) => users.map(Map.dbUser);
    static assignedUsers = (users) => users.map((user) => ({
        id: user.userId || '',
        userType: user.userType,
    }));
    static unknownDate = (date) => date !== undefined && typeof date === 'string' && date !== ZERO_TIME ? new Date(date) : undefined;
}
exports.Map = Map;
class PermissionsMapping {
    ...
    static use = (role) => new PermissionsMapping(role);
    map = () => {
    ...
    permissionFromWeaviate = (permission) => {
