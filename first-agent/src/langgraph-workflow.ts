...
type WorkflowState = {
  ...
  history: Array<{ node: string; result: any; timestamp: number }>;
...
class WorkflowGraph {
  ...
  private edges: Map<string, string[]>; // 从节点到可能的下一节点
   ...
   * 添加历史记录
   */
  private addHistory(node: string, result: NodeResult): void {
   ...
   * 获取执行历史
   */
  getHistory(): WorkflowState['history'] {
...
// 开始节点
class StartNode implements WorkflowNode {
  name = "start";
...
请以 JSON 格式返回评分，例如：
{
  ...
  "overall": 8
    ...
    let scores: any = { accuracy: 5, completeness: 5, readability: 5, usefulness: 5, overall: 5 };
...
// 内容增强节点
class EnhanceNode implements WorkflowNode {
  name = "enhance";
  private model: ChatAnthropic;
...
// 批准节点
class ApproveNode implements WorkflowNode {
  name = "approve";
...
// 拒绝节点
class RejectNode implements WorkflowNode {
  name = "reject";

  async execute(state: WorkflowState): Promise<NodeResult> {
    console.log("❌ 内容已拒绝");

    const reason = state.data.qualityCheck?.passed === false
      ? "质量检查未通过"
      : state.data.safetyCheck?.passed === false
 ...
 * 创建内容审核工作流
 */
function createContentModerationWorkflow(): WorkflowGraph {
 ...
 * 内容审核工作流演示
 */
async function contentModerationDemo() {
  ...
  try {
    ...
    for (const testCase of testCases) {
      ...
      await new Promise(resolve => setTimeout(resolve, 2000)); // API限流延迟
    }
 ...
 * 自定义工作流演示
 */
async function customWorkflowDemo() {
  console.log("\n\n🔧 自定义工作流演示 - 文章生成流程");
  console.log("=".repeat(60) + "\n");
  ...
  // 研究节点
  class ResearchNode implements WorkflowNode {
    name = "research";
    private model: ChatAnthropic;
...
请以简洁的要点形式回答。`;
  ...
  // 大纲节点
  class OutlineNode implements WorkflowNode {
    name = "outline";
    private model: ChatAnthropic;
...
请提供一个结构清晰的文章大纲。`;
  ...
  // 写作节点
  class WriteNode implements WorkflowNode {
    name = "write";
    private model: ChatAnthropic;
...
请以 JSON 格式返回评估结果。`;
      ...
      try {
        ...
        const jsonMatch = content.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
          review = { ...review, ...JSON.parse(jsonMatch[0]) };
        }
      } catch (e) {
  ...
  const finalState = await workflow.execute(15);
...
async function main() {
  // 运行内容审核工作流演示
  await contentModerationDemo();

  // 运行自定义工作流演示
  await customWorkflowDemo();
}
