...
- 引用具体的片段编号支持你的回答`;
   ...
   * 获取知识库信息
   */
  getKnowledgeBaseInfo(): { documentCount: number } {
    return {
...
`;
    ...
    // 将知识库内容添加到向量存储
    console.log("📚 正在创建知识库...\n");
    await ragAgent.ingestText(sampleKnowledge, {
    ...
    for (const query of queries) {
      await new Promise(resolve => setTimeout(resolve, 500)); // 添加延迟避免 API 限制
      await ragAgent.query(query);
    }
 ...
 * 自定义文档 RAG 演示
 */
async function customDocRagDemo() {
  console.log("\n📄 自定义文档 RAG 演示");
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
...
`;
  ...
  await ragAgent.ingestText(customDoc, {
