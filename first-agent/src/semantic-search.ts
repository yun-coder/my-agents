...
class SemanticSearchEngine {
   ...
   * 语义搜索
   */
  async semanticSearch(query: string, topK: number = 5): Promise<Array<{ doc: Document; score: number }>> {
    console.log(`\n🔍 语义搜索: "${query}"`);
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  ...
  async hybridSearch(
    ...
    topK: number = 5
  ): Promise<Array<{ doc: Document; semanticScore: number; keywordScore: number }>> {
    console.log(`\n🔍 混合搜索: "${query}"`);
    if (filters) {
    ...
    for (const result of semanticResults) {
      // 应用分类过滤
      if (filters?.category && result.doc.metadata.category !== filters.category) {
        continue;
      }
      ...
      if (filters?.tags && filters.tags.length > 0) {
        const hasTag = filters.tags.some(tag =>
          result.doc.metadata.tags.includes(tag)
        );
        if (!hasTag) continue;
      }
   ...
   * 获取统计信息
   */
  getStats(): { totalDocuments: number; categories: Set<string> } {
 ...
 * 格式化搜索结果
 */
function formatSearchResults(results: Array<any>): string {
  if (results.length === 0) {
    return "未找到相关结果";
  }
...
async function semanticSearchDemo() {
  ...
  try {
    ...
    const semanticResults = await engine.semanticSearch("AI 和语言模型");
    console.log(formatSearchResults(semanticResults));

    // 示例 2: 混合搜索
    console.log("🔍 示例 2: 混合搜索（语义 + 关键词）");
    console.log("=".repeat(50));
    const hybridResults = await engine.hybridSearch("TypeScript 编程");
    console.log(formatSearchResults(hybridResults));

    // 示例 3: 带过滤条件的搜索
    console.log("🔍 示例 3: 带分类过滤的搜索");
    console.log("=".repeat(50));
    const filteredResults = await engine.hybridSearch(
      "开发",
      { category: "AI技术" },
      3
    );
    ...
    console.log("=".repeat(50));
    const tagFilteredResults = await engine.hybridSearch(
      "开发工具",
      { tags: ["LLM", "AI"] },
      5
    );
    ...
    const relevantDocs = await engine.semanticSearch(searchQuery, 3);
