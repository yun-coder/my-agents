...
class MemoryPersistenceStore {
   ...
   * 创建新对话
   */
  createConversation(userId: string, title?: string, metadata?: Record<string, any>): Conversation {
...
class ConversationalAssistant {
  ...
  constructor(
    memoryStore: MemoryPersistenceStore,
    systemPrompt?: string
  ) {
   ...
   * 开始新对话
   */
  startConversation(userId: string, title?: string): string {
 ...
 * 格式化对话列表
 */
function formatConversations(conversations: Conversation[]): string {
 ...
 * Memory 持久化演示
 */
async function memoryPersistenceDemo() {
  ...
  try {
    ...
    const assistant = new ConversationalAssistant(
      memoryStore,
      "你是一个专业的编程导师，擅长解释技术概念，提供编程建议，并帮助解决编程问题。"
    );
    ...
    for (const question of questions) {
      ...
      const answer = await assistant.chat(conversationId, question);
    ...
    if (restoredConversation) {
      ...
      const followUpAnswer = await assistant.chat(conversationId, followUpQuestion);
    ...
    const summary = await memoryStore.summarizeConversation(conversationId);
    if (summary) {
      console.log("📋 对话总结:");
      console.log(summary);
    }
    ...
    const assistant2 = new ConversationalAssistant(
      memoryStore,
      "你是一个旅行规划助手，擅长提供旅行建议和规划行程。"
    );

    const travelConversationId = assistant2.startConversation("user456", "日本旅行规划");
    await assistant2.chat(travelConversationId, "我计划去日本旅行，有什么建议吗？");
