...
class ResearcherAgent {
    constructor() {
        ...
        this.description = "负责收集和分析信息，提供背景资料和研究数据";
...
class AnalystAgent {
    constructor() {
        this.name = "分析师";
        this.role = "analyst";
        this.description = "负责分析数据，识别模式和趋势，提供洞察";
...
class StrategistAgent {
    constructor() {
        ...
        this.description = "负责制定策略和规划，设计解决方案";
...
class ReviewerAgent {
    constructor() {
        this.name = "审核员";
        this.role = "reviewer";
        this.description = "负责审核和验证结果，确保质量";
...
class CoordinatorAgent {
    constructor() {
        ...
        this.description = "负责协调各 Agent 的工作，分配任务，整合结果";
        this.model = createMiniMaxModel("claude-3-5-haiku-20241022", 0.3);
        this.agents = [];
    }
    registerAgent(agent) {
    ...
    execute(task, context) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // 分析任务并分配给合适的 Agent
            const taskAnalysis = yield this.analyzeTask(task);
            console.log(`📋 任务分析:\n${taskAnalysis}\n`);
            // 执行多阶段工作流
            const results = yield this.executeWorkflow(task);
            // 整合结果
            const integration = yield this.integrateResults(task, results);
...
用简洁清晰的格式回答。`;
    ...
    executeWorkflow(task) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (researcher) {
                const researchResult = yield researcher.execute(task);
                results.set("research", researchResult);
                yield this.delay(1000); // 避免API限流
            }
            // 阶段 2: 分析阶段
            const analyst = this.agents.find(a => a.role === "analyst");
            if (analyst && results.has("research")) {
                const analysisResult = yield analyst.execute(task, results.get("research"));
            ...
            if (strategist && results.has("analysis")) {
                ...
                const strategyResult = yield strategist.execute(task, combinedContext);
            ...
            if (reviewer && results.has("strategy")) {
                ...
                const reviewResult = yield reviewer.execute(task, fullContext);
                results.set("review", reviewResult);
            }
            return results;
        });
    }
    integrateResults(task, results) {
        return __awaiter(this, void 0, void 0, function* () {
...
function multiAgentDemo() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const result1 = yield coordinator.execute(task1);
            console.log("\n📊 最终整合结果:");
            console.log("─".repeat(60));
            console.log(result1);
            console.log("─".repeat(60));
            ...
            const result2 = yield coordinator.execute(task2);
            console.log("\n📊 最终整合结果:");
            console.log("─".repeat(60));
            console.log(result2);
            console.log("─".repeat(60));
 ...
 * Agent 能力展示
 */
function agentCapabilitiesDemo() {
    return __awaiter(this, void 0, void 0, function* () {
        console.log("\n\n🔧 Agent 能力展示");
        console.log("=".repeat(60));
        ...
        console.log("─".repeat(60));
        const research = yield researcher.execute(sampleTask);
        console.log(`\n🔍 研究员视角:\n${research.substring(0, 200)}...\n`);
        yield new Promise(resolve => setTimeout(resolve, 1000));
        const analysis = yield analyst.execute(sampleTask, research);
        console.log(`📊 分析师视角:\n${analysis.substring(0, 200)}...\n`);
        yield new Promise(resolve => setTimeout(resolve, 1000));
        const strategy = yield strategist.execute(sampleTask, `${research}\n\n${analysis}`);
        console.log(`🎯 策划师视角:\n${strategy.substring(0, 200)}...\n`);
        yield new Promise(resolve => setTimeout(resolve, 1000));
        const review = yield reviewer.execute(sampleTask, strategy);
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        // 运行多 Agent 协作演示
        yield multiAgentDemo();
        // 运行 Agent 能力展示
        yield agentCapabilitiesDemo();
    });
}
// 执行主函数
main();
